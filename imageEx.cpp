...
//------------------------------
// .image image file structs
//------------------------------
typedef struct
{
	uint32_t sig;				//32 1E ED 32
	uint32_t unk[3];
	uint32_t framePtr;			//Point to frameItem
	uint32_t imgPtr;			//Point to imgHeader
} imageHeader;
...
bool splitFiles(const char* cFilename)
{
	...
	size_t namepos = sName.find(".image");
	...
	//grab header
	imageHeader ih;
	memcpy(&ih, fileData, sizeof(imageHeader));
	
	//Grab image data
	imgHeader img;
	memcpy(&img, &fileData[ih.imgPtr], sizeof(imgHeader));
	
	vector<FIBITMAP*> images;
	for(int i = 0; i < img.numPieces; i++)
	{
		imgDataPtr idp;
		memcpy(&idp, &fileData[ih.imgPtr + sizeof(imgHeader) + i*sizeof(imgDataPtr)], sizeof(imgDataPtr));
		
		uint32_t imgSize = 1 << (idp.size);
		
		FIBITMAP* result = imageFromPixels(&fileData[ih.imgPtr + idp.offset], imgSize, imgSize);
		images.push_back(result);
	}
	
	//Grap piecing data
	frameItem fi;
	memcpy(&fi, &fileData[ih.framePtr], sizeof(frameItem));
	
	vector<rect> fiRect;
	for(int j = 0; j < fi.numRects; j++)
	{
		rect r;
		memcpy(&r, &fileData[ih.framePtr+fi.rectStartOffset+j*sizeof(rect)], sizeof(rect));
	...
	{
		memcpy(&ri, &fileData[ih.framePtr+sizeof(fi)+j*sizeof(rectImg)], sizeof(rectImg));
		rcImgs.push_back(ri);
	}
	...
	//{
		...
		vec sz;
		sz.x = sz.y = 0;
		for(int j = 0; j < rih.size(); j++)
		{
			sz.x = max(sz.x, (uint16_t)(rih[j].rc.bl.x + 1));
			sz.x = max(sz.x, (uint16_t)(rih[j].rc.br.x + 1));
			sz.x = max(sz.x, (uint16_t)(rih[j].rc.ul.x + 1));
			sz.x = max(sz.x, (uint16_t)(rih[j].rc.ur.x + 1));
			sz.y = max(sz.y, (uint16_t)(rih[j].rc.bl.y + 1));
			sz.y = max(sz.y, (uint16_t)(rih[j].rc.br.y + 1));
			sz.y = max(sz.y, (uint16_t)(rih[j].rc.ul.y + 1));
			sz.y = max(sz.y, (uint16_t)(rih[j].rc.ur.y + 1));
		}
		...
		for(int j = 0; j < rih.size(); j++)
		{
			fixRect(rih[j], sz.x, sz.y, FreeImage_GetWidth(images[rih[j].img]));
		}
	...
	//{
		//Create final image
		FIBITMAP* result = FreeImage_Allocate(sz.x, sz.y, 32);
		
		//Piece images
		for(int j = 0; j < rih.size(); j++)
		{
			//Grab piece
			FIBITMAP* imgPiece = FreeImage_Copy(images[rih[j].img], rih[j].rc.ul.u, rih[j].rc.ul.v, rih[j].rc.br.u, rih[j].rc.br.v);
			//If this needs to be rotated
			if(rih[j].rotAmt)
			{
				FIBITMAP* tmp = imgPiece;
				imgPiece = FreeImage_Rotate(imgPiece, rih[j].rotAmt);
			...
			FreeImage_Paste(result, imgPiece, rih[j].rc.ul.x, rih[j].rc.ul.y, 255);
